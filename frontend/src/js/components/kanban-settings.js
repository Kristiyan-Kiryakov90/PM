...
import {
  ...
  deleteStatus,
  reorderStatuses,
  getStatusesWithTaskCounts,
} from '../services/status-service.js';
import { showSuccess, showError } from '../utils/ui-helpers.js';
...
async function loadStatuses() {
  try {
    statuses = await getStatusesWithTaskCounts(currentProjectId);
  } catch (error) {
...
async function saveStatusOrder(newOrder) {
  try {
    await reorderStatuses(currentProjectId, newOrder);
    ...
    showError('Failed to reorder statuses');
...
async function handleSaveStatus() {
  ...
  try {
    if (statusId) {
      ...
      await updateStatus(parseInt(statusId, 10), statusData);
    ...
    showError(error.message || 'Failed to save status');
  }
}
...
async function handleDeleteStatus() {
  ...
  try {
    ...
    showError(error.message || 'Failed to delete status');
  }
}
