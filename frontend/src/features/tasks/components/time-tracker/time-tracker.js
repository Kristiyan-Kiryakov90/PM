 ...
 * Main component for timer widget and time entry management
...
export async function initTimeTracker(container, taskId) {
  ...
  function setupEventListeners() {
    ...
    if (toggleBtn) {
      toggleBtn.addEventListener('click', () => toggleManualEntryForm(container));
    }

    // Save manual entry
    const saveBtn = container.querySelector('#saveManualEntryBtn');
    if (saveBtn) {
      saveBtn.addEventListener('click', () => handleSaveManualEntry(container, taskId, render));
    }
  ...
  async function handleStartTimer() {
    try {
      ...
      uiHelpers.showSuccess('Timer started');
      await render();
    } catch (error) {
      console.error('Error starting timer:', error);
      uiHelpers.showError(error.message || 'Failed to start timer');
    }
  }
  ...
  async function handleStopTimer() {
    try {
      ...
      uiHelpers.showSuccess('Timer stopped');
      await render();
    } catch (error) {
      console.error('Error stopping timer:', error);
      uiHelpers.showError('Failed to stop timer');
    }
  }
  ...
  async function handleDeleteEntry(entryId) {
    ...
    try {
      ...
      uiHelpers.showSuccess('Time entry deleted');
      await render();
    } catch (error) {
      console.error('Error deleting entry:', error);
      uiHelpers.showError('Failed to delete time entry');
    }
  }
