...
export async function renderTagManager(container) {
  ...
  async function render() {
    ...
    const tagsWithUsage = await Promise.all(
      tags.map(async (tag) => {
        const usage = await tagService.getTagUsage(tag.id);
              ...
              <div class="tag-form">
                ...
                <div class="tag-form-group">
                  ...
                  <div class="tag-form-color-grid">
                    ${tagService.TAG_COLORS.map(color => `
                      <div
                ...
                <div class="tag-form-preview">
                  ...
                  <span class="tag-badge" id="tagPreview" style="background-color: ${tag?.color || tagService.TAG_COLORS[11].value}; color: white;">
  ...
  function setupFormListeners(existingTag) {
    ...
    let selectedColor = existingTag?.color || tagService.TAG_COLORS[11].value;
    ...
    saveBtn?.addEventListener('click', async () => {
      ...
      if (!name) {
        uiHelpers.showError('Tag name is required');
        return;
      }
      ...
      try {
        ...
        uiHelpers.showError(error.message || 'Failed to save tag');
      }
    });
  ...
  async function handleDeleteTag(tagId) {
    ...
    const usage = await tagService.getTagUsage(tagId);
    ...
    try {
      ...
      uiHelpers.showError('Failed to delete tag');
    }
  }
