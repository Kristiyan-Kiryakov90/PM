...
export const checklistService = {
  async getTaskChecklists(taskId) {
    try {
  ...
  async toggleChecklistItem(itemId) {
    try {
      ...
      // Toggle it
      return await this.updateChecklistItem(itemId, {
   ...
   */,
  async getChecklistProgress(checklistId) {
    try {
      const { data, error } = await supabase
        .from('checklist_items')
        .select('is_completed')
   ...
   */,
  async getTaskChecklistsProgress(taskId) {
    try {
   ...
   * @returns {Promise<void>}
   */,
  async reorderChecklistItems(checklistId, itemOrders) {
    try {
