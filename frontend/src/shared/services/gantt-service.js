...
export const ganttService = {
  ...
  async autoScheduleTasks(projectId) {
    try {
      ...
      const tasks = await this.getGanttTasks({ project_id: projectId });
      ...
      for (const update of updates) {
        try {
          const result = await this.updateTaskDates(update.id, {
  ...
  async rebuildAutoDependencies(projectId) {
    ...
    const tasks = await this.getGanttTasks({ project_id: projectId, sort_by: 'gantt_position' });
